...
outputNameTemplate = 'cvqa-sameQuestionDataset-sameObjects-leastCommonQuestionsBoth-%s.csv'
...
for qId,item in enumerate(question_answers):
	...
	for q in item['qas']:
		...
		questionToQuestionIndexes[question].append(qId)
		for l in list1:
			if l in words:
...
for l in questionTemplates:
	...
	for question in questionTemplates[l].items():
		print '\t[%d] Question:[%s]' % (question)
		questionIds = questionToQuestionIndexes[question]
		qId = random.sample(xrange(len(questionIds)), 1)
...
# 	possibleObjects = [o for o in flags if (not(flags[o]))]
# 	if len(possibleObjects) < subsetCount:
# 		continue
