...
def filter(questionRows):
	...
	for i,questionRowIndex in enumerate(questionRows.keys()):
	...
	questionDf = pd.DataFrame(questionRows.values())
	...
	output = {}
	outputItems = questionDf[questionDf['selected'] == 1].T.to_dict().values()
	for i,o in enumerate(outputItems):
		output[i] = o
...
# list2 = {'canine':['dog'], 'feline':['cat'], 'person':['man','woman','boy','girl'], 'automobile':['car','bus','taxi','jeep','van','minivan','ambulance','motorcycle','truck'], 'fauna':['shrub','flower','tree']}
# list1 = ['dog','cat','horse','man','woman','boy','girl']
# numberOfObjects = len(objects)

# outputFilePath = os.path.join(baseFolder,'generated-data/cvqa-sameQuestionDataset-subset-list3-test.csv')
outputJsonFilePath = os.path.join(outputFolder,'cvqa-sameQuestionDataset-subset-list3-train.json')
...
for l in questionTemplates:
	...
	for (k,v) in sortedTemplates[:10]:
		selectedTemplates[l].append(k)

# print 'Reducing number of instances of selected questions...'

# for q in questionToQuestionIndexes:
# 	print q
# 	indices = questionToQuestionIndexes[q]
# 	print 'Original size: [%d]' % (len(indices))
# 	K = int(len(indices) * 0.10)
# 	indices = [ indices[i] for i in sorted(random.sample(xrange(len(indices)), K))]
# 	print 'New size: [%d]' % (len(indices))
# 	questionToQuestionIndexes[q] = indices
...
# pd.DataFrame(selectedQuestions).to_csv(outputFilePath)

sortedQuestionsJson = {}
for i,s in enumerate(selectedQuestions):
	sortedQuestionsJson[str(i)] = s

sortedQuestionsJson = filter(sortedQuestionsJson)

with open(outputJsonFilePath, 'w') as output:
	output.write(json.dumps(sortedQuestionsJson))
