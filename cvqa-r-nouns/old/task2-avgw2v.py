...
for i,questionRowIndex in enumerate(questionKeys):
	...
	for currentWi in range(len(questionWords)):
		...
		ques =sum(questionFeature)/float(len(questionFeature))
		cap = sum(captionFeature)/float(len(captionFeature))

		X.append(np.concatenate((ques,cap),0))
...
model=Sequential()
model.add(Dense(200,input_dim=wordVectorSize*2,activation='relu'))
...
model.fit(X_train,y_train, nb_epoch=numberOfEpochs, verbose=1, callbacks=[normalizedLoss])
